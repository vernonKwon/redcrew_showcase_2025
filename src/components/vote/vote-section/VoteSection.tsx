...
export default function VoteSection() {
  ...
  return (
    <section className="vote-section">
      <div className="vote-section__background">
        <div className="vote-section__gradient"></div>
      </div>
      
      <div className="vote-section__container">
        <Link href="/" className="vote-section__back-link">
          <ArrowLeft size={20} />
          홈으로 돌아가기
        </Link>

        <div className="vote-section__header">
          <h1 className="vote-section__title">
            <span className="vote-section__title-highlight">게스트 팀</span> 인기투표
          </h1>
          <p className="vote-section__description">
        ...
        <div className="vote-section__categories">
            ...
            return (
              <div key={category.id} className="vote-section__category">
                <div className="vote-section__category-header">
                  <div className="vote-section__category-icon">
                    <IconComponent size={24} />
                  </div>
                  <div className="vote-section__category-info">
                    <h2 className="vote-section__category-title">{category.title}</h2>
                    <p className="vote-section__category-description">{category.description}</p>
                    <div className="vote-section__category-stats">
                      <Users size={16} />
                ...
                <div className="vote-section__options">
                  {category.options.map((option) => {
                    ...
                    return (
                      <div
                        key={option.id}
                        className={`vote-section__option ${isSelected ? 'vote-section__option--selected' : ''} ${isVoted ? 'vote-section__option--voted' : ''}`}
                        onClick={() => handleVote(category.id, option.id)}
                      >
                        <div className="vote-section__option-image">
                          <img src={option.imageUrl} alt={option.title} />
                          <div className="vote-section__option-year">{option.style}</div>
                        </div>
                        
                        <div className="vote-section__option-content">
                          <h3 className="vote-section__option-title">{option.title}</h3>
                          <div className="vote-section__option-meta">
                            <span className="vote-section__option-location">
                              <MapPin size={14} />
                              {option.location}
                            </span>
                        ...
                        {isSelected && !isVoted && (
                          <div className="vote-section__option-selected">
                            ✓
                          </div>
                        )}
                      </div>
                ...
                {selectedVotes[category.id] && !hasVoted[category.id] && (
                  <div className="vote-section__submit">
                    <button
                      onClick={() => submitVote(category.id)}
                      className="vote-section__submit-button"
                    >
                      <Star size={20} />
                      투표하기
                    </button>
                ...
                {hasVoted[category.id] && (
                  <div className="vote-section__voted">
                    <div className="vote-section__voted-message">
                      <Heart size={20} />
        ...
        <div className="vote-section__footer">
          <div className="vote-section__footer-content">
            <h3 className="vote-section__footer-title">투표 기간</h3>
            <div className="vote-section__footer-info">
              <Clock size={20} />
              <span>2024년 11월 1일 ~ 2024년 12월 10일</span>
            </div>
            <p className="vote-section__footer-note">
